...
export class Chunk {
    ...
    verticalCollision(cameraLocation, upwards) {
        const topleftx = this.x - this.size / 2;
        const toplefty = this.y - this.size / 2;
        const base = Math.round(cameraLocation.y - Config.PlayerHeight);
        const top = Math.round(cameraLocation.y);
    ...
    apply2x2KernelToSquareMatrix(kernel, matrix) {
        let matrixDim = Math.floor(Math.sqrt(matrix.length));
        if (Math.pow(matrixDim, 2) !== matrix.length) {
    ...
    generateValueNoiseOfUpdatedSize(size, octaves) {
        // Step 1: Allocate noise array with (size + 1) x (size + 1) elements
        let seed = `${this.x}_${this.y}_${size}`;
        let rng = new Rand(seed);
        let paddedSize = size + 2;
        let cubePositionsF32 = new Float32Array(Math.pow(paddedSize, 2));
        let scaleFactor = (1.0 / ((size / this.size) * (Math.pow(2, (octaves - 1))))) / 2;
        ...
        let noiseArrays = seeds.map(seed => {
            ...
            let newRNG = new Rand(seed);
            let arr = new Float32Array(size * size);
        ...
        // TopLeft
        cubePositionsF32[0] = noiseArrays[7][Math.pow(size, 2) - 1];
        // Bottom
        for (let i = 1; i < size + 1; i++) {
            let idx = (i - 1);
        ...
        // Bright
        cubePositionsF32[Math.pow((size + 2), 2) - 1] = noiseArrays[4][0];
        // BLeft
        cubePositionsF32[(size + 2) * (size + 1)] = noiseArrays[5][size - 1];
        // TRight
        cubePositionsF32[size + 1] = noiseArrays[6][Math.pow(size, 2) - size];
        ...
        let retArray = new Float32Array(this.size * this.size);
    ...
    shouldDrawBasedOnDensity(i, j, k) {
    ...
    perlinDensity(gridSpace, coord) {
        ...
        let v000 = this.unit_vec3d(x0, y0, z0);
        let v001 = this.unit_vec3d(x0, y0, z1);
        let v010 = this.unit_vec3d(x0, y1, z0);
        let v011 = this.unit_vec3d(x0, y1, z1);
        let v100 = this.unit_vec3d(x1, y0, z0);
        let v101 = this.unit_vec3d(x1, y0, z1);
        let v110 = this.unit_vec3d(x1, y1, z0);
        let v111 = this.unit_vec3d(x1, y1, z1);
        let afin000 = ((coordN.x - x0) * v000.x + (coordN.y - y0) * v000.y + (coordN.z - z0) * v000.z);
        let afin001 = ((coordN.x - x0) * v001.x + (coordN.y - y0) * v001.y + (coordN.z - z1) * v001.z);
        let afin010 = ((coordN.x - x0) * v010.x + (coordN.y - y1) * v010.y + (coordN.z - z0) * v010.z);
        let afin011 = ((coordN.x - x0) * v011.x + (coordN.y - y1) * v011.y + (coordN.z - z1) * v011.z);
        let afin100 = ((coordN.x - x1) * v100.x + (coordN.y - y0) * v100.y + (coordN.z - z0) * v100.z);
        let afin101 = ((coordN.x - x1) * v101.x + (coordN.y - y0) * v101.y + (coordN.z - z1) * v101.z);
        let afin110 = ((coordN.x - x1) * v110.x + (coordN.y - y1) * v110.y + (coordN.z - z0) * v110.z);
        let afin111 = ((coordN.x - x1) * v111.x + (coordN.y - y1) * v111.y + (coordN.z - z1) * v111.z);
    ...
    generateCubes() {
        ...
        for (let i = 3; i < octaves; i++) {
            ...
            // 0-100)
            let blockWidth = Math.floor((this.size) / (Math.pow(2, i)));
            let something = this.generateValueNoiseOfUpdatedSize(blockWidth, octaves);
            // Verify that the size of the arrays match
            // Adds the noise to the this.heightMap
            this.heightMap = this.heightMap.map((value, index) => {
